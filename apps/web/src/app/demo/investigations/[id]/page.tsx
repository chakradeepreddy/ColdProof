...
function Section({ step, title, badge, accentColor = 'experiment', className, children }: {
  ...
  return (
    <div ref={ref} className={cn(
      'reveal bg-surface border border-border rounded-xl overflow-hidden mb-4 transition-all duration-300',
      'hover:border-border/70 hover:shadow-[0_4px_24px_-10px_rgba(0,0,0,0.5)]',
      accentCls,
      className
    )}>
...
export default function InvestigationDetailPage() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/demo/investigations/${id}`,
          {}
        );
  ...
  if (loadingData && !investigation) return null;
    ...
    <>
      <div className="px-6 py-8 max-w-5xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-7 animate-fade-in">
          <Link href="/demo/investigations" className="inline-flex items-center text-secondary/60 hover:text-primary text-sm transition-colors duration-150 gap-1.5 group">
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
            <div>
              ...
              <div className="flex items-center gap-3 mb-2 flex-wrap">
                <div className="flex items-center gap-3"><h1 className="text-2xl font-bold text-primary tracking-tight">Demo Investigation</h1><span className="px-2 py-0.5 text-[10px] font-bold tracking-widest uppercase bg-experiment/10 text-experiment border border-experiment/20 rounded-full">Read Only</span></div>
          ...
          <div className="p-5">
            {aiExplanation ? (
              ...
              <div className="flex flex-col items-center py-6 gap-4 text-center">
                <div className="w-10 h-10 rounded-full bg-experiment/8 border border-experiment/20 flex items-center justify-center">
                  <svg className="w-4 h-4 text-experiment/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <p className="text-sm text-secondary/50 max-w-sm leading-relaxed">
                  No AI explanation was generated for this investigation.
                </p>
              </div>
            )}
          </div>
